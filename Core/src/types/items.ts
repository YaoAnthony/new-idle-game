...
export type ItemDefinition = {
   ...
   *
   * `"hand"` = 单手拿在右手上：锄头、水壶这类**工具**（种植系统 期 6）。挂在
   * 手掌的挂点上，胳膊怎么抡它怎么走；怎么握、怎么用由表现层的工具类
   * （`Game3D/Tools`）按 `tool.toolType` 决定，这里只说"拿法"。
   */
  carry?: "overhead" | "hand";
  ...
  /**
   * **一包种子**：种下去长出什么。有这一块的物品能在田上播种。
   *
   * 和"家具是带 placement 块的物品"同一个路数：**能力块而不是新类型**。
   * 种子不另立 ItemCategory，因为它在背包里、能堆叠、能送人，就是个物品；
   * "能种"是它多出来的一项能力。
   *
   * **只写 cropId**（2026-09-17）：多久熟、收几个、几段造型全在 `Data/crops`
   * 的作物表里——种子是"入口"，作物才是"内容"，两处各写一份迟早走散。
   * 收获时果实和种子一起给，所以种子不是买来的消耗品，是作物自己的循环。
   */
  seed?: { cropId: string };
  ...
  tool?: {
    ...
    /**
     * **这件工具够得着多远**，单位是**格**（田里一格一株，1 格 = 1 米）。
     * 缺省 0 = 只作用于对准的那一格。
     *
     * ## 是半径，不是"多少格"
     *
     * 半径是几何量，"几格"是它的推论——以后加"一行""十字形"这类形状
     * 时，半径这个语义还能用，格数就不行了。切比雪夫半径 1 = 3×3 共九格，
     * 正是"一次喷 9 个区域"（2026-09-17 之前田是整块算的，那时广口壶写的
     * 是半径 3 米；田内按格分之后单位换成格，值改回 1）。
     *
    ...
    power?: number;
    /**
     * 装得下几格水（水壶）。此刻还剩几格在物品堆上（`ItemStackState.charges`）——
     * 定义是"能装多少"，堆是"现在有多少"，和耐久同一个路数。没有这一项的
     * 工具不消耗水。买来是空的，去井边装。
     */
    capacity?: number;
  };
