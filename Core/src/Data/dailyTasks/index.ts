...
/**
 * 每日确定性伪随机。**抽签不能用 Math.random()**——同一天里反复开关面板
 * 必须抽出同一批，否则玩家会一直重开面板直到抽到最容易的那四条。
 *
 * 算法本体 2026-09-17 搬进 `logic/random`（巨大果实、访客、远行都在用，
 * 算法不是内容）。这里保留同名转发，调用方零改动。
 */
export { drawDeterministic, hashSeed, seededRandom } from "../../logic/random.js";
