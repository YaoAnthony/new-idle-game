...
import { iconUrl, itemIconUrl } from "../../Assets/icons";
...
/** 保留资源的图标键。真物品走 `items/<id>` 那条约定，不列在这里 */
 ...
 * （商店卡片退化成名字，不留空洞）。
 *
 * 金币不在 `items/` 下是因为它压根不是物品：`findItemDefinition("gold")`
 * 是空的。保留资源各自指一张图，和 `materialNameKey` 各自指一句文案
 * 是同一个路数。
 */
export function materialIconUrl(itemId: ItemId): string | undefined {
  const reserved = RESERVED_ICONS[itemId];
  if (reserved) return iconUrl(reserved);
  return findItemDefinition(itemId) ? itemIconUrl(itemId) : undefined;
}
