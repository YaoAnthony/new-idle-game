...
export type DoorDefinition = {
  ...
  /**
   * 推锁着的门时**弹的对话**（2026-09-09）。填了就不念 lockedTextKey，改成拉起这段
   * 对话（玩家自言自语）——每推一次弹一次，门开了自然就没有了。
   * 开场的大门用它："还是先把家里收拾一下吧"。
   */
  lockedDialogueId?: string;

  /**
   * 门上可以贴一张条子（14 开场）：这里填**旗子的键**，旗子的值是信件表里的 letterId。
   * 有条子时门的气泡是"？"，按 F 先读条子（走信箱的信纸），读过旗子清空、门恢复正常。
   * 读没读过就是旗子在不在——不另加存档字段。
   */
  noteFlag?: string;
