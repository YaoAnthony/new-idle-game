...
export const dailyBoardDefinition: DailyBoardDefinition = {
   ...
   *
   * 2026-08-25 短暂改成过 5，当天改回——那次讨论说的"一天最多五件"
   * 指的是**行动系统**，不是这块板；这里被误伤了。留这行是因为
   * 光看数字看不出它被动过，下次有人想调之前该知道 5 不是没试过。
   *
   * 改这一个数就够：进度条、满格判定、补抽、经济不变量全是引用它算的，
   * 代码里没有第二处写着这个数字——包括机器胸前那排装饰豆
   * （`recipes/dailyBoard`）现在也按它推导，原来是四颗写死的。
   */
  taskCount: 4,
