...
/**
 * 做完之后"还没做完？再来 N 分钟"（2026-09-13，专注模式 01·甲）。
 *
 * 分钟数是玩家自己填的，这里只给**框里预先写着的那个数**；上下限不另设，
 * 直接用那类行动自己的 `durationMinutes`——延长出来的一轮就是一条普通行动，
 * 合法时长不该因为"是接着做的"而变。
 *
 * 30 是用户定的。它也是日记本写计划时分钟框的默认值，两处看到同一个数。
 */
export const actionExtendTuning = {
  defaultMinutes: 30,
};
