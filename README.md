...
`SAVE_SCHEMA_VERSION = 48`（迁移链 v2 起步，最新一条见 `Data/Save/migrations.ts` 文件尾）。IndexedDB，每个槽三键（主档 / 备份 / 冲突），**双份轮写**：写主档前先把当前主档复制到备份，读不出来自动回退（回退必须告诉玩家）。

**四个存档槽**（2026-09-07）：本地 A / B / C ＋ 云端 ×1，地位相同——都能进、下载成文件、删除，空了能新建或导入。区别只有一条：云槽跟账号绑定，**只有玩云槽时才走云同步**，A/B/C 永远纯本地。键由 `Data/Save/slots.ts` 的 `keysForSlot` 按槽推导（`world.a` / `world.a.backup` …），**云槽沿用没有后缀的老键**（`world`），因为同步基准记的是"本机这份主档和云端的关系"，换键等于让已登录玩家吃一个假冲突框。别处不许自己拼键。设计与回归清单见 [`gpt设计稿/存档槽位/README.md`](Frontend-3D/gpt设计稿/存档槽位/README.md)。
