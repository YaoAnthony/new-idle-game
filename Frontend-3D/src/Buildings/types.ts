...
export type BuildingDefinition = {
  ...
  /**
   * **这栋不立工地**：确认下去当场就建好，石傀儡不用走过来。
   * 升级同理（一堵墙建得瞬间、升级却要等人跑一趟，那是两套规矩）。
   *
   * 木墙就是这一类（用户定，2026-08-23："木墙不需要傀儡过来建设，
   * 直接就会建好"）。理由是**它是按段刷的**：围一圈院子是十几二十段，
   * 每一段都排一次队、都等石傀儡走过去，那不是施工演出是罚站。
   * 部落冲突也正是这么切的——城墙是全场唯一不占建筑工人的建筑。
   *
   * 所以判据不是"便宜"也不是"小"，是**一次会摆很多个**。给新建筑打这个
   * 标之前先问这一句：玩家会一口气摆十个吗？不会的话，那段等待正是
   * 这套系统要演的东西，别抹掉。
   */
  skipsSite?: boolean;
