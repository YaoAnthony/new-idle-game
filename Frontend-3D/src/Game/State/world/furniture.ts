...
export function seedInitialFurniture(): void {
  ...
  const pieces: Array<{
    ...
    /*
     * **床回来了，壁炉没有**（2026-09-09）。
     *
     * 2026-08-23 用户定过"刚进来的小房间里不要放壁炉和床"，那时开场是
     * "搬家第一天"。新开场是**在魔女的屋子里醒来**：镜头从枕头上睁眼、
     * 坐起来（见 Game3D/World/OpeningIntro），所以得有一张床让人躺——
     * 用户指定就是 furniture_bed，位置在屋里西北角（世界 (−9, 15.6) 附近）。
     *
     * 格 (7, 0) 朝北，2×3 占 x7..8 / y0..2：贴北墙、挨着卧室那扇小窗，
     * 离左上角的石台（x0..3）隔着三格。朝北 = 枕头那头贴墙（模型的
     * 枕头在本地 −z，房本地的北就是 −z，和房子转没转向无关）。
     * 地铺相应从纸箱里拿掉了（Core Data/loot）。
     *
     * 壁炉仍然不放：屋外那根烟囱是外壳的一部分（`WitchRoof.buildChimney`），
     * "有烟囱没壁炉"读起来是"炉子还没搬来"，不是穿帮。
     */
    {
