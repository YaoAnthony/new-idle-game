...
**铺地（协议 v17，地面系统）**：`ground_set { roomId, cell: { x, y }, groundId | null }` 广播一格地面
的**绝对值**（这格现在是什么，null = 草地），重复 / 乱序都收敛；`grounds` 刷新切片（格 → 地面 id 的稀疏表）
收敛。服务端查 roomId 非空 ≤ 64、格是两个整数、groundId 字符串 ≤ 64 或 null。居民寻路按它算代价，
房客端也一样（切片到了就一致）。

**工具动作（协议 v16，种植系统 期 6）**：手势加 `tool_use`，带 `tool: { itemId, use, at? }`——
哪件工具、工具类自己定的动作名（swing / pour / fill）、对着世界里哪一点。远端按 `itemId`
找回同一个工具类、按名字播同一个动作，**只播动作不改状态**（状态走 `building_state_set`）。
服务端查 `itemId` 非空 ≤ 64、`use` 非空 ≤ 32、`at` 是两个有限数。

**建筑状态（协议 v15，种植系统 期 5）**：`building_state_set` 广播一栋楼的
实例状态补丁（`{ instanceId, patch }`，合并进 `BuildingPlacement.state`）——
田里哪格翻了 / 种了 / 浇了 / 收了、罐里存了多少、小店抽屉里的钱。**patch 自带
整块真相**（田是整块 `farm`，不是"哪格 +1"），重复 / 乱序都收敛到同一份；
`buildings` 切片的刷新也听 `building_state_changed`，房主随后的整片刷新拉平。
服务端只查 patch 是对象、顶层 ≤ 32 键、深度 ≤ 6（patch 自己算第 1 层；田是 5 层）、≤ 8 KB，不查游戏规则。
巨大果实的判定和雨水只在自己的世界里掷 / 浇（房客的节拍器不写）。
...
| `sync:residents` | 房主 → 全房（volatile，2 Hz，只发有变化的） | 关键帧：位置 / 朝向 / 头的偏转 / 正在做的动词 / 隐身。房客偏差 < 0.6 m 忽略、0.6~3 m 插值、> 3 m 直接放；朝向和头的偏转照房主的转过去（走路中朝向跟自己的路） |
...
**注视（协议 v14，居民系统 21）**：说话的双方互相看着对方。身体的朝向本来就在 `heading` 里；
头相对身体的偏转多一个可选字段 `headYaw`（弧度，0 不带）——`sync:transform`（玩家）和
`sync:residents`（活物）都有。谁在看谁是房主的决定，不上网线，网线上只有结果。
服务端对 `transform.headYaw` 只查有限、|值| ≤ π；关键帧照旧不逐字段校验。
