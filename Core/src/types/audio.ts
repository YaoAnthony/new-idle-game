...
export type AudioProfileDefinition = {
  ...
  /**
   * 白噪音台上归到哪一条推子。不填 = 自己一条。
   *
   * 台子上的行是**从当前在响的循环现推出来的**，不是一张写死的清单；
   * 但"在响的循环"这个粒度有时候太细：森林的白天和夜晚是两条不同的
   * 素材（`ambience_forest_day` / `_night`），玩家把白天那条静音了，
   * 天一黑声音自己回来——从他的角度看就是设置失效了。
   *
   * 所以归组这件事交给数据说：昼夜两条填同一个 group，推子就是一条。
   * 写成代码里的分支（"名字里带 forest 的算一组"）迟早在下一个
   * 昼夜素材上失效，而这里加一行就够。
   */
  mixerGroup?: string;
